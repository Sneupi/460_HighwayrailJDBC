...
public class Prog3 {
    ...
    public static void executeA(Connection dbconn) throws SQLException {

        String sql = "SELECT '1980' AS table_year, COUNT(*) AS row_count FROM venegasg1.\"1980\" "
                + "UNION "
                + "SELECT '1995' AS table_year, COUNT(*) AS row_count FROM venegasg1.\"1995\" "
                + "UNION "
                + "SELECT '2010' AS table_year, COUNT(*) AS row_count FROM venegasg1.\"2010\" "
                + "UNION "
                + "SELECT '2025' AS table_year, COUNT(*) AS row_count FROM venegasg1.\"2025\" ";

        Prog3.executeQuery(sql, dbconn);
    }

    // Display incident count by state (top 10, descending) for given year
    public static void executeB(Connection dbconn, String year) throws SQLException {

        String sql = "SELECT * FROM ("
                + "SELECT state_name, COUNT(*) AS incident_count FROM venegasg1.\"" + year + "\" "
                + "GROUP BY state_name "
    ...
    public static void executeC(Connection dbconn, String year1, String year2) throws SQLException {
        ...
        String sql = "SELECT * FROM ("
                ...
                + "FROM (SELECT state_name, COUNT(*) AS incident_count FROM venegasg1.\"" + year1
                + "\" GROUP BY state_name) y1 "
                + "JOIN (SELECT state_name, COUNT(*) AS incident_count FROM venegasg1.\"" + year2
    ...
    public static void executeD(Connection dbconn, String year1, String year2, String field) throws SQLException {
        ...
        String sql = "SELECT * FROM ("
                ...
                + "     y2.incident_count AS incidents_" + year2 + ", "
                + "     ROUND(100.0 * ((y1.incident_count - y2.incident_count) / (1.0 * y1.incident_count)), 2) AS percent_drop "
                + "FROM (SELECT " + field + ", COUNT(*) AS incident_count FROM venegasg1.\"" + year1
                + "\" GROUP BY " + field + ") y1 "
                + "JOIN (SELECT " + field + ", COUNT(*) AS incident_count FROM venegasg1.\"" + year2
                + "\" GROUP BY " + field + ") y2 "
    ...
    // prints helpful CLI commands to user
    public static void printHelp() {
        System.out.println(
            "\n\t--------------------------------------------------------------\n\n"
            + "\t(All other input will be ran as SQL statements w/ output)\n\n"
            + "\t?tuple                     - Display highwayrail tuple fields & types\n\n"
            + "\t?a                         - Display incident count by year\n\n"
            + "\t?b <year>                  - Display incident count by state \n"
            + "\t                             (top 10, descend) for given year\n\n"
            + "\t?c <year1> <year2>         - Display largest % drop in \n"
            + "\t                             incident count by state (top 5, \n"
            + "\t                             descend) between given years\n\n"
            + "\t?d <year1> <year2> <field> - Display largest % drop in \n"
            + "\t                             incident count by field (top 10, \n"
            + "\t                             descend) between given years\n\n"
            + "\t--------------------------------------------------------------\n\n"
            + "\tNOTE:\n\n"
            + "\tCourse TAs can find my tables explicitly under \"venegasg1\":\n\n"
            + "\t\tvenegasg1.\"1980\"\n"
            + "\t\tvenegasg1.\"1995\"\n"
            + "\t\tvenegasg1.\"2010\"\n"
            + "\t\tvenegasg1.\"2025\"\n\n"
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String dbURL = "jdbc:oracle:thin:@aloe.cs.arizona.edu:1521:oracle";
            ...
            System.err.println("*** ClassNotFoundException:  "
                    ...
                    + "For CSC 460 on Lectura, run:\n"
                    + "\texport CLASSPATH=/usr/lib/oracle/19.8/client64/lib/ojdbc8.jar:${CLASSPATH}");
            System.exit(-1);
        } catch (SQLException e) {
            e.printStackTrace();
            System.err.println("Unable to get connection to DB");
        }
        ...
        while (true) {
            ...
            try {
                ...
                if (query.equalsIgnoreCase("exit"))
                    ...
                    Prog3.executeA(dbconn);


                } else if (query.toLowerCase().startsWith("?b")) {
                    String year = query.split("\\s+")[1];
                    Prog3.executeB(dbconn, year);


                } else if (query.toLowerCase().startsWith("?c")) {
                    String year1 = query.split("\\s+")[1];
                    String year2 = query.split("\\s+")[2];
                    Prog3.executeC(dbconn, year1, year2);


                } else if (query.toLowerCase().startsWith("?d")) {
                    String year1 = query.split("\\s+")[1];
                    String year2 = query.split("\\s+")[2];
                    String field = query.split("\\s+")[3];
                    Prog3.executeD(dbconn, year1, year2, field);
